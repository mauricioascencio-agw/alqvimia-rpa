// 🎯 ELEMENT SPY RPA - POPUP SCRIPT (Firefox Compatible)
// Interfaz de usuario de la extensión

console.log('🟢 Element Spy RPA Popup cargado (Firefox)');
...
function loadState() {
  ...
  browserAPI.storage.local.get(['projectConfig'], (result) => {
  ...
  // Cargar estadísticas
  browserAPI.storage.local.get(['eventCount', 'objectCount'], (result) => {
...
async function startRecording() {
  ...
  await browserAPI.storage.local.set({ projectConfig: config });
...
async function saveRecording() {
  ...
  try {
    ...
    const result = await browserAPI.storage.local.get(['projectConfig', 'eventLogs', 'capturedObjects']);
    ...
    if (data.success) {
      ...
      // Limpiar storage
      await browserAPI.storage.local.remove(['eventLogs', 'capturedObjects', 'eventCount', 'objectCount']);
      eventCount = 0;
      objectCount = 0;
      updateStats();
    } else {
...
// Mostrar mensaje (Seguro para Firefox Add-ons)
